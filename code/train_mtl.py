...
from load_data_only_marker import *
...
class Dataloader(pl.LightningDataModule):
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            train_label = [train_df["main_labels"].to_numpy(), train_df["sub_labels"].to_numpy, train_df["obj_labels"].to_numpy]
            # train, validation split
            # test size를 바꾸거나, random_state를 바꾸면 validation이 바뀝니다.
            train_idx, val_idx = train_test_split(np.arange(len(train_label[0])), test_size=0.1, random_state=42, stratify=train_label[0])
            train_data, val_data = train_df.loc[train_idx], train_df.loc[val_idx]
            train_label, val_label = train_data[["main_labels", "sub_labels", "obj_labels"]].to_numpy(), val_data[["main_labels", "sub_labels", "obj_labels"]].to_numpy()
...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr, vocab_size, loss=torch.nn.CrossEntropyLoss, warmup_steps=500):
        super().__init__()
        ...
        hsize = self.plm.config.hidden_size

        self.classifier = torch.nn.Sequential(
                torch.nn.Linear(hsize, hsize),
                torch.nn.Dropout(p=0.1, inplace=False),
                torch.nn.Linear(hsize, 30)
        )
        self.subwordClassifier = torch.nn.Sequential(
                torch.nn.Linear(hsize, hsize),
                torch.nn.Dropout(p=0.1, inplace=False),
                torch.nn.Linear(hsize, 2)
        )
        self.objwordClassifier = torch.nn.Sequential(
                torch.nn.Linear(hsize, hsize),
                torch.nn.Dropout(p=0.1, inplace=False),
                torch.nn.Linear(hsize, 6)
        )
    def forward(self, x):
        x = self.plm(input_ids=x['input_ids'], attention_mask=x['attention_mask'], token_type_ids=x['token_type_ids'])['pooler_output']
        return self.classifier(x), self.subwordClassifier(x), self.objwordClassifier(x)
        
    def training_step(self, batch, batch_idx):
        x, y = batch
        main_pred, sub_pred, obj_pred = self(x)
        loss = (self.loss_func(main_pred, y[:,0]) + self.loss_func(sub_pred, y[:,1]) + self.loss_func(obj_pred, y[:,2])) / 3
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_labels.append(y[:, 0])
...
if __name__ == '__main__':
    ...
    wandb_logger = WandbLogger(
        log_model=False, 
        name=f'{args.model_name.replace("/","-")}_{args.wandb_name}_{args.learning_rate:.2e}_{args.batch_size}_{args.max_epoch}_{args.use_LSTM}', # wandb run name
    ...
    # model and trainer
    model = Model(args.model_name, args.learning_rate, vocab_size, loss=loss, warmup_steps=args.warmup_steps)
